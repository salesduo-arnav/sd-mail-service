...
|----------|------|-------------------|
| **core-platform** | micro-tool host | `*.trial_started`, `*.plan_purchased`, `integration.connected`, `checkout.initiated` |
| **AI Creative Studio** (sd-listings-optimizer) | micro-tool | `generation.completed`, `activity` |
...
1. core-platform emits `creative_studio.trial_started` for the user.
2. sd-mail-service matches the **"no integration after 1 day"** workflow: it schedules a send for **+1 day**, marked to cancel if `integration.connected` arrives.
3. If the user connects within the day, core emits `integration.connected` → sd-mail-service **cancels** the pending send.

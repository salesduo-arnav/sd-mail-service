...
|-----------|-----------|------|
| `creative_studio.trial_started` | core-platform | A trial/subscription for the Creative Studio tool begins (Stripe webhook path where `notifyTrialStarted` fires today) |
| `creative_studio.integration.connected` | core-platform | An `IntegrationAccount` for the org becomes `connected` |
| `creative_studio.generation.completed` | studio (optimizer) | A generation (image / A+ / SEO) finishes — the studio already consumes credits at these points |
| `creative_studio.activity` | studio | Any meaningful use (bumps `last_seen_at`) |
| `creative_studio.plan_purchased` | core-platform | Trial converts to paid |
| `creative_studio.checkout.initiated` / `.checkout.completed` | core-platform billing | **Phase 2** — user clicks pay CTA / completes payment |
...
- **Workflow** `welcome` · trigger `creative_studio.trial_started` · category `onboarding` · audience `event_subscriber`
- **Steps:** `[{ send: welcome }]`
...
- **Workflow** `no_integration_1d` · trigger `creative_studio.trial_started` · category `onboarding`
- **Steps:** `delay 1d` → `cancel_on integration.connected` → `send`
...
sequenceDiagram
  ...
  N->>N: schedule +1d send, guard=integration.connected
  alt connects in time
    Core->>N: integration.connected → cancel run
...
- **Workflow** `no_generation_2d` · trigger `creative_studio.integration.connected` · category `onboarding`
- **Steps:** `delay 2d` → `cancel_on generation.completed` → `send`
...
- **Workflow** `abandoned_checkout_1d` · trigger `creative_studio.checkout.initiated` · category `billing` · audience `org_owner`
...
- **Workflow** `trial_ended` · trigger `creative_studio.trial_started` · category `billing` · audience `org_owner`
- **Steps:** `delay until:trial_ends_at` → `cancel_on plan_purchased` → `send`
...
The trial-end send is scheduled to the absolute `trial_ends_at` carried in the trigger event's `data`, and is canceled if the user converts (`plan_purchased`) first.
...
- **Workflow** `inactive_14d` · trigger `creative_studio.activity` (and `generation.completed`) · category `reengagement`
...
- **core-platform:** emit `creative_studio.trial_started` (at the existing `notifyTrialStarted` hook), `creative_studio.integration.connected` (when an `IntegrationAccount` becomes connected), `creative_studio.plan_purchased`. Phase 2: `checkout.initiated`/`checkout.completed`.
- **studio:** emit `creative_studio.generation.completed` and/or `creative_studio.activity` at generation flows (where credits are already consumed).

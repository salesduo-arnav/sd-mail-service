...
- **Two message classes** (`messages.type`, `templates.type`):
  - **marketing** — lifecycle/nudge mail (workflow-driven). Carries a signed unsubscribe link (scoped to subscriber + category) that opens a confirmation page — the link itself is a `GET` that never changes state, so mail-security link scanners can't unsubscribe a user; the actual unsubscribe is a `POST` (button submit, or `List-Unsubscribe` + `List-Unsubscribe-Post` one-click). Blocked by preference opt-out **and any** suppression (unsubscribe, complaint, hard bounce). Fully unsubscribable.

...
export const getProduct = asyncHandler(async (req: Request, res: Response) => {
    ...
    const keys = await ApiKey.findAll({
        ...
        attributes: ['id', 'name', 'last_used_at', 'revoked_at', 'created_at'],
        order: [['created_at', 'DESC']],
    });
    res.json({ product, api_keys: keys });
});
...
export const createApiKey = asyncHandler(async (req: Request, res: Response) => {
    ...
    res.status(201).json({ id: key.id, name: key.name, api_key: plaintext, note: 'shown once — store it now' });
});
